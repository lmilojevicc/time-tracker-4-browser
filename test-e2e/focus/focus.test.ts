...
describe('Focus Block', () => {
    ...
    test('allow mode', async () => {
        ...
        await startSession(popupPage, { template: 'focus', mode: 'allow', cond: [MOCK_URL] })
        ...
        // 5. Abort to end session and unblock
        await popupPage.bringToFront()
    ...
    test('block mode', async () => {
        const popupPage = await context.openPopupPage('focus')
        await startSession(popupPage, { template: 'focus', mode: 'block', cond: [MOCK_URL] })
        ...
        expect(info.abortVisible).toBeTruthy()
