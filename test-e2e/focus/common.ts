...
async function selectTemplate(page: Page, template: tt4b.focus.Template): Promise<void> {
    await page.waitForSelector('.el-card', { timeout: 3000 })
    const cards = await page.$$('.el-card')
    const card = cards[template === 'focus' ? 0 : 1]
    if (!card) throw new Error(`Template card not found: ${template}`)
    await card.click()
    await page.waitForSelector('.cond-editor', { timeout: 500 })
}

async function setMode(page: Page, mode: tt4b.focus.Mode): Promise<void> {
    const buttons = await page.$$('.el-radio-button')
    const button = buttons[mode === 'allow' ? 0 : 1]
...
export async function startSession(page: Page, config: tt4b.focus.Config): Promise<void> {
    ...
    await setMode(page, config.mode)
